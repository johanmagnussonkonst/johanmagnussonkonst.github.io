...
const renameInnerFolders = function (inData) {
  inData.forEach((child) => {
    if (child.children) {
      child.children.forEach((grandChild) => {
        if (
          grandChild.name.includes(".jpg") ||
          grandChild.name.includes(".png") ||
          grandChild.name.includes(".txt")
        ) {
          // do nothing
        } else {
          const newPath =
            grandChild.path.substring(0, grandChild.path.lastIndexOf("/")) +
            "/" +
            grandChild.name;
